...
function translateContent(text, from, to, retryCount) {
  ...
  //     }
  //   }
  let apiKey = "AIzaSyDnLSlI88t85D2L83igJvUKKu1Ocxq0eaY";
  let modelId = "gemini-2.0-flash";
  let generateContentApi = "generateContent";
  let url = `https://generativelanguage.googleapis.com/v1beta/models/${modelId}:${generateContentApi}?key=${apiKey}`;
  let requestBody = {
    contents: [
      {
        role: "user",
  ...
  //   //   const response = callGeminiModel(apiKey, text);
  //   if (response) {
  //     return Response.success(response);
  //   } else {
  //     return Response.success("Lỏ vl :))");
  //   }
